...
const formatCurrency = (n) => new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(+n || 0);
...
function renderTable() {
  ...
  inventory.forEach((item) => {
    ...
    row.innerHTML = `
      <td><input class="w-full rounded-md border border-gray-300 py-2 px-2" type="text" value="${escapeHtml(item.manufacturer)}" data-key="manufacturer" data-id="${item.id}"/></td>
      <td><input class="w-full rounded-md border border-gray-300 py-2 px-2" type="text" value="${escapeHtml(item.model)}" data-key="model" data-id="${item.id}"/></td>
      <td><input class="w-full rounded-md border border-gray-300 py-2 px-2" type="number" min="0" step="0.01" value="${item.price}" data-key="price" data-id="${item.id}"/></td>
      <td><input class="w-11 h-11 p-0 rounded-md border border-gray-300 cursor-pointer" type="color" value="${item.color}" data-key="color" data-id="${item.id}"/></td>
      <td>
        <button class="inline-flex items-center justify-center bg-red-600 text-white w-11 h-11 rounded-lg" data-action="delete" data-id="${item.id}" title="Delete">
  ...
  getEls('input[data-id]', tbody).forEach((input) => {
    ...
    input.addEventListener('input', debounce(() => { saveInventory(); queueChart(); }, 200));
  });
